...
async function callFoodPhotoAI(base64){
  ...
  const resp = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=' + encodeURIComponent(apiKey), {
    method: 'POST',
    headers: {
...
el('photoInput').addEventListener('change', async (e)=>{
  ...
  if(!apiKey){
    statusBox.innerHTML = '<div class="warn">Adiciona tua chave de API do Google Gemini em ⚙️ Configurações pra usar esse recurso.</div>';
    return;
  }
  ...
  try{
    ...
    if(String(err.message).includes('400') || String(err.message).includes('API_KEY_INVALID')) msg = 'Chave de API inválida — confere em Configurações.';
    else if(String(err.message).includes('429')) msg = 'Limite de uso gratuito atingido, tenta em instantes.';
    else if(err.message) msg += ' (' + String(err.message).slice(0,120) + ')';
